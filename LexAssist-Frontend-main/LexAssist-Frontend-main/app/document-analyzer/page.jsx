...
export default function DocAnalyzerPage() {
    ...
    const handleUseInChatbot = async () => {
        ...
        try {
            // Step 1: Create a new chat session to get its ID
            const { data: newChat } = await apiClient.post("/chats");
            
            // Step 2: Prepare the document context with the special 'system' role
            const documentPayload = {
                message_content: `CONTEXT FROM DOCUMENT ("${file.name}"):\n---\n${originalText}\n---`,
                role: "system" // <<< THE CRUCIAL FIX
            };
            
            // Step 3: Send the document context as a SILENT system message to the new chat
            await apiClient.post(`/chats/${newChat._id}/messages`, documentPayload);
            
            // Step 4: Add a user-friendly starter message on the frontend only
            const starterMessage = {
                _id: `info-${Date.now()}`,
                role: 'assistant',
                content: `I have read the document "${file.name}". What specific questions do you have about it?`
            };
            
            // Step 5: Set the context for navigation, now including the starter message
            setDocumentContext({
                text: originalText,
                filename: file.name,
                chatId: newChat._id,
                starterMessage: starterMessage // Pass the starter message
            });
            
            // Step 6: Navigate to the chatbot
            router.push('/chatbot');
        ...
        <ProtectedRoute>
            <div className="min-h-[calc(100vh-128px)] bg-[var(--legal-cream)] p-6 md:p-12">
                <div className="max-w-4xl mx-auto">
                    <h1 className="text-3xl font-semibold text-[var(--legal-ink)] mb-6 text-center">Document Analyzer</h1>
